  ...
  do
    ...
    do
      ...
      do
        ...
        callbacks.model_checkpoint_base.filename=CRNN_puig_src_$dataset-$img_size-$batch_size-$lr  \ 
        trainer.deterministic=False \ 
        callbacks.model_checkpoint_id.filename="\${callbacks.model_checkpoint_base.filename}_ID" \ 
        +trainer.limit_train_batches=1.0 \ 
        +trainer.limit_val_batches=1.0 \ 
        callbacks/heldout_targets=[$rest_datasets] \
        logger.wandb.offline=False"
        echo $call_script
        # eval "$call_script"
      done
