...
class HTRCharClassifierAR(LightningModule):
    def __init__(
        self,
        net: torch.nn.Module,
    ...
    def training_step(self, batch: Tuple[torch.Tensor, torch.Tensor], batch_idx: int) -> torch.Tensor:
        ...
        # labels[labels < 3] = 0 # Convert 0, 1 and 2 to 0 (padding token) to 0
        ...
        images_masked_ = images_masked.round()
        ...
        images_masked_segmenter = (images_masked_segmenter > 0.5).float() 

        # Reshape to original images_masked shape
        images_masked_segmenter = images_masked_segmenter.reshape(images.shape[0], -1, 1, images_masked.shape[-2], images_masked.shape[-1])
        preds = self.net(images_masked_segmenter).squeeze(-1)
        ...
        # print(f'Images masked segmenter (after reshape) shape: {images_masked_segmenter.shape} to predict with the model')

        preds_from_masked = self.net(images_masked_).squeeze(-1)         
        preds_ = preds_from_masked.reshape(images_masked.shape[0], -1, preds_from_masked.shape[-1])#.softmax(-1).argmax(-1)
        preds_ = preds_.flatten(0, 1)
        
        print(f'Preds.shape: {preds_.view(images.shape[0], -1, preds_.shape[-1]).shape}')
        print(f'Labels.shape: {labels_.view(images.shape[0], -1).shape}')

        # labels_ = labels_.reshape(images_masked.shape[0], -1)
        loss = self.criterion(preds_, labels_)
        cer_batch = 0.0

        preds_ = preds_.reshape(images_masked.shape[0], -1, preds_.shape[-1]).softmax(-1).argmax(-1)
        ...
        if batch_idx < 10:
          ...
          print(f'Preds_ [:10]: {preds_.view(images.shape[0], -1, preds_.shape[-1])[:10]}')
          print(f'Labels_ [:10]: {labels_.view(images.shape[0], -1)[:10]}')
    ...
    def validation_step(self, batch: Tuple[torch.Tensor, torch.Tensor], batch_idx: int, dataloader_idx: int = None) -> None:
        ...
        images_masked = (images_masked > 0.5).float()
        images_masked = images_masked.reshape(images.shape[0], images_masked.shape[1], 1, images.shape[-2], images.shape[-1])

        print(f'Images masked shape (after flatten): {images_masked.shape}')
        preds = self.net(images_masked).squeeze(-1)
