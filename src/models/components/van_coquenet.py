...
# from https://github.com/FactoDeepLearning/VerticalAttentionOCR/blob/master/basic/models.py
...
class VAN_Coquenet(Module):
    def __init__(
      ...
      super(VAN_Coquenet, self).__init__()
      self.cnn_encoder = FCN_Encoder(dropout)
      self.tokenizer = tokenizer
      self.img_reduction = (32, 8)
      self.adaptive_max_pool = nn.AdaptiveMaxPool2d((1, None))
      self.vocab_size = tokenizer.vocab_size
      self.conv = Conv2d(256, self.vocab_size+1, kernel_size=(1, 1), padding=(0, 0)) # +1 for CTC blank token
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # print(f'ADAPTIVE MAX POOL OUTPUT: {x.shape}')
        x = self.conv(x)
