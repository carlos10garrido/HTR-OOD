...
class SegTransformerMaskLitModule(LightningModule):
    ...
    def training_step(self, batch: Tuple[torch.Tensor, torch.Tensor], batch_idx: int) -> torch.Tensor:
        ...
        labels = labels.round()
        # Invert labels to get a binary mask where 1 is this pixel have text and 0 is this pixel is background
        # labels = 1 - labels
        # labels = labels > 0.5
        # labels = labels.float()
        ...
        # print(f'Count non-zeros and zeros in labels[0] {torch.count_nonzero(labels[0])} {torch.count_nonzero(1 - labels[0])}')
        ...
        if batch_idx < 10:
            ...
            for i in range(5):
              ...
              # print(f'Images pred shape: {images_pred[i].shape}')
              # print(f'Image original shape: {images[i].shape}')
              # Apply a threshold to the predicted images to get a binary mask
              # images_pred[i] = (images_pred[i] > 0.5).float().round()
              # Apply the binary mask to the original images 
              ...
              self.metric_logger.log_images(images_masked_i, 'training_images_masked2')
    ...
    def validation_step(self, batch: Tuple[torch.Tensor, torch.Tensor], batch_idx: int, dataloader_idx: int = None) -> None:
        ...
        preds = preds.logits
        # Round preds to get a binary mask
        preds = (preds > 0.5).float().round()

        print(f'preds shape: {preds.shape}')

        # images_pred = preds.reshape(preds.shape[0], preds.shape[1], 1, images.shape[2]//2, images.shape[3]//2) * 255.0
        if batch_idx < 50:
          images_pred = preds.reshape(preds.shape[0], preds.shape[1], 1, images.shape[2], images.shape[3]) 
