...
# from src.data.seg_mask_datamodule import SegDataModule
from src.data.char_datamodule import CharDataModule
from src.models.transformer_seg_mask_module_e2e import SegTransformerMaskLitModuleE2E
...
def train(cfg: DictConfig) -> Tuple[Dict[str, float], Dict[str, Any]]:
    ...
    # Init data module
    log.info("Instantiating DataModule...")
    datamodule: LightningDataModule = SegDataModule(
    ...
    # Check if there exists a model checkpoint through callbacks.model_checkpoint.filename
    # If there is, load the filepath, if there isn't, instantiate the model
    ...
    print(f'Callbacks: {trainer.callbacks}')
    ...
    model = SegTransformerMaskLitModuleE2E.load_from_checkpoint(ckpt_path) if ckpt_path is not None else model
    # Train the model
    trainer.fit(model, datamodule.train_dataloader(), datamodule.val_dataloader(), ckpt_path=None)
